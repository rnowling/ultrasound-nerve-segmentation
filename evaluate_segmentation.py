...
def train_and_predict(args):
    ...
    pred_masks = np.load(os.path.join(args.input_dir, 'imgs_pred_mask_test.npy'))

    pred_masks = pred_masks.astype('float32')
    scaled = pred_masks / 255.

    dice = np_dice_coef(imgs_mask_test,
                        scaled)

    acc = accuracy_score(scaled.reshape(-1),
                         imgs_mask_test.reshape(-1))
